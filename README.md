...
从 v1.0.1 起，额外使用独立线程约每 10 ms 检测 F10 按下状态，并保留停止请求；启动延迟和每轮点击前都会检查。此备用路径不依赖 `WM_HOTKEY`，也不要求松开 Ctrl/Alt/Shift。它仍受 Windows 桌面访问限制，极短按键可能被轮询漏过；不能保证在锁屏、安全桌面或系统拦截环境中生效。状态栏会显示由“F10 热键”还是“F10 按键检测”触发停止。
